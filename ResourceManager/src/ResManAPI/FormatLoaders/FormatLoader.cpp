...
std::string FormatLoader::extractFile(const char* path, size_t check) {
	...
	if (index == -1)
		RM_DEBUG_MESSAGE("Couldn't find file '" + pathInPackage + "'. Have you spelled the filename correctly?", 1);
	...
	mz_zip_reader_extract_to_mem(&archive, index, buffer, (size_t)fileStat.m_uncomp_size, 0);
